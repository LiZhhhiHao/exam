...
export default {
  ...
  methods: {
    ...
    getChangeLabel(val) {
      ...
      let answeridx = this.selectarr.findIndex(
        (item) =>
          item.questionId == this.topic[1][this.index].questionId 
      );
      if (this.selectarr.length === 0) {
        this.selectarr.push(current);
      } else if (answeridx != -1) {
        this.selectarr[answeridx].answer = val;
      } else {
        this.selectarr.push(current);
      }
    ...
    getJudgeLabel(val) {
      ...
      let answeridx = this.fillinarr.findIndex(
        (item) =>
          item.questionId == this.topic[3][this.index].questionId 
      );
      if (this.juedgearr.length === 0) {
        this.juedgearr.push(current);
      } else if (answeridx != -1) {
        this.juedgearr[answeridx].answer = val;
      } else {
        this.juedgearr.push(current);
      }
      console.log(this.juedgearr);
    },
    previous() {
      //上一题
      this.index--;
    ...
    commit() {
      ...
      // myself
      let arr = [];
      let currentfill = this.fillAnswer;
      // console.log(this.topic[2]);
      currentfill.forEach((ele, idx) => {
        let tmp = ele.slice(0, 3);
        arr.push({
          questionId: this.topic[2][idx].questionId,
          studentAnswer: tmp,
        });
      });
      this.fillinarr = arr;
      ...
      this.answerlist = {
        ...
        fillin: this.fillinarr,
        juedge: this.juedgearr,
      };
      console.log(this.answerlist);
